...
import { CompetitorAnalysisSection } from "@/components/report/competitor-analysis-section";
  ...
  return (
    <div className="min-h-screen py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Combined Idea + Score Card */}
        <div className="bg-surface rounded-2xl p-6 mb-6">
          {/* Row 1: Idea */}
          <div className="mb-6">
            <h2 className="text-sm font-medium text-yc-orange uppercase tracking-wide mb-2">
              Idea
            </h2>
            <p className="text-lg text-foreground">{reportData.idea}</p>
          </div>

          {/* Row 2: Score circle + class on left, motivation on right */}
          <ScoreDisplay score={report.score} scoreReasoning={reportData.scoreReasoning} />
        </div>

        {/* Quick Take */}
        {reportData.initialTake && (
          <div className="bg-surface rounded-2xl p-6 mb-6 border-l-4 border-yc-orange">
            <h3 className="text-sm font-medium text-yc-orange uppercase tracking-wide mb-2">
              Quick Take
            </h3>
            <p className="text-foreground/80 italic">{reportData.initialTake}</p>
          </div>
        )}

        {/* Competitor Analysis - YC, Other Competitors, Product Hunt */}
        <CompetitorAnalysisSection
          ycSummary={sections.historical.summary}
          ycCompanies={sections.historical.companies}
          outcomeCounts={sections.historical.outcomeCounts}
          competitorsSummary={sections.competitors?.summary}
          competitors={sections.competitors?.companies}
          phSummary={sections.productHunt?.summary}
          phProducts={sections.productHunt?.products}
        />
